...
@section('page-subtitle', 'Buat layanan baru ke katalog')
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="glass-card rounded-2xl border border-white/10 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Konsultasi Perencanaan Keuangan"
            ...
            <div>
                ...
                <input type="text" name="short_description" value="{{ old('short_description') }}" placeholder="Ringkasan 1-2 kalimat..."
            ...
            <div>
                ...
                <textarea name="description" rows="6" required placeholder="Jelaskan layanan secara detail..."
                    class="w-full px-4 py-3 bg-white/5 border @error('description') border-red-500/50 @else border-white/10 @enderror rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-sm resize-none">{{ old('description') }}</textarea>
